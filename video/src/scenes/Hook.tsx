...
const SQUIGGLE_AT = 36;
const ERROR_AT = 45; // timeline.ts の tick と揃える

export const Hook = () => {
  const frame = useCurrentFrame();
  const errorPop = usePop(ERROR_AT, 11);
  const cursorOn = Math.floor(frame / 8) % 2 === 0;
  // 最後の半拍でエラー行へ寄る。次の場面のライザーと重ねる。
  const push = interpolate(frame, [BAR + BEAT * 2.5, BAR * 2], [1, 1.35], {
    easing: Easing.in(Easing.cubic),
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const drift = interpolate(frame, [0, BAR * 2], [1, 1.04]);
  ...
  return (
    <AbsoluteFill>
      ...
      >
        ...
        >
          ...
          <div style={{ padding: PAD, position: "relative" }}>
            ...
            {frame > 26 && cursorOn && frame < SQUIGGLE_AT && (
              <span
                style={{
                  position: "absolute",
        ...
        <Squiggle
          width={7 * CHAR}
          progress={progress(frame, SQUIGGLE_AT, 10)}
      ...
      <Caption
        ...
        start={20}
        style={{ left: 180, top: 740, opacity: 1 - progress(frame, BAR + BEAT * 2.5, 8) }}
      />
    </AbsoluteFill>
  );
};
