...
/**
 * 入力文字数からトークン数を見積もる係数（1 token あたりの文字数）。
 *
 * Gemini のトークナイザを Worker 上で正確に再現はできない。送信前に入力を
 * 拒否するには見積もりが要るので、**実際より多めに数える側**へ倒す。
 * 日本語は1文字がおおむね1トークン前後になるため、1 とする。
 * 実際の消費量は応答の `usageMetadata` から取り、蓄積にはそちらを使う。
 */
export const INPUT_CHARS_PER_TOKEN = 1;

/** 送信前の入力トークン見積もり。実消費ではなく、拒否の判定にだけ使う。 */
export function estimateInputTokens(text: string): number {
  return Math.ceil(text.length / INPUT_CHARS_PER_TOKEN);
}
