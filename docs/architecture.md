...
`checks:generate` は AI Gateway とは別の口である（#184）。`ai/responses` は `selection`
（コード選択）を前提にしたスキーマで、問題生成には渡すべき `selection` が無い。
入力は Concept の定義（`id` / `label` / `language` / `summary` と周辺 Concept の表示名）だけで、
**利用者個人の情報とコード本文は渡さない。** そのため生成した問題は個人データではなく、
全利用者で使い回せる静的コンテンツになる。回数上限との関係は
「確認問題の生成は回数上限の対象外」を参照。**まだ公開していない**（同節の「順序の制約」）。

生成に失敗したとき（2問揃わない、正解が選択肢に無い、別の Concept が返る、途中で切れる）は、
**失敗として 502 を返す。** 本文には種別（`reason`）と、画面へそのまま出せる説明（`message`）を
載せる。画面は問題が作れなかったことを利用者へ伝え、**黙って空の出題にしない**
（`.agents/rules/rules.md` RULE-004）。上流の応答の断片は本文へ載せず、ログへ残す。
...
`POST /v1/checks:generate`（#184）は `ai_usage` を加算しない。**この経路だけが例外である。**

理由は、生成した問題を全利用者で使い回すことである（#185）。生成の入力は Concept の定義だけで
個人に依存せず、1 Concept につき1回の呼び出しで2問を作る。Concept は 148 件の有限集合なので、
**生成量は利用者数に比例せず、システム全体で自然に閉じる。**
全員が使う問題を作るコストを、最初にアクセスした利用者の枠から引く理由が無い。

**外すのは回数の勘定だけである。** 代わりの歯止めを明示する。

| 機構                                          | 何を守るか             | 生成での扱い       |
| --------------------------------------------- | ---------------------- | ------------------ |
| `ai_usage`（日 15 回 / 月 150 回）            | 外部プロバイダの単価   | **対象外にする**   |
| `PROFILE_RATE_LIMITER`（利用者ごと 30 回/分） | 頻度。叩き放題にしない | 維持する           |
| model allowlist と1回あたりの入力・出力の上限 | 1回あたりの単価        | 維持する           |
| 既知の `conceptId` だけを受理する             | 生成先の集合           | 148 件に限る       |
| 保存済みがあれば生成しない（#185）            | 生成の総回数           | セットで有効化する |
| 上流への単発リクエストのタイムアウト（30 秒） | 応答の来ない呼び出し   | 掛ける（RULE-001） |

消費トークンは `ai_usage` に載らないため、**生成のコストが残る唯一の場所は
`check generation completed` の構造化ログである。** 148 件を埋めるまでの総量はここから数える。

**順序の制約: #185 より先に有効化しない。** 歯止めの本体は「保存済みなら生成しない」であり、
それを実装するのは #185 である。生成だけを先に公開すると `ai_usage` の外で毎回生成が走る経路が
開くため、`apps/api/src/app.ts` へはまだ繋いでいない（`app.test.ts` が公開されていないことを
検査する）。画面の導線も出さない。
