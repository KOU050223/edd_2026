...
**ユーザー1人あたり、入力と出力を合算したトークン数で数える。**

| 期間                    | 上限               |
| ----------------------- | ------------------ |
| 1日（UTC 基準）         | **50,000 tokens**  |
| 1ヶ月（暦月、UTC 基準） | **500,000 tokens** |

入出力を合算する理由は、**数えられるものだけを上限にする**ためである。
入力と出力を別々に持つと、どちらが先に尽きたのかを利用者へ説明する手間が増えるうえ、
単価の違い（出力は入力の5倍）を利用者が意識しなければ残量を予測できなくなる。
合算なら「残りいくら」が1つの数字で言える。

単価差は**上限額の見積もりを最悪ケースで取る**ことで吸収する（下の試算）。

日次と月次の両方を置く理由は、片方では守れないためである。
日次だけなら月末まで毎日上限を使い切られたときの月額が読めない。
月次だけなら初日に1ヶ月分を使い切られ、残り29日が使えなくなる。

#### 何回分の質問にあたるか

上限は利用者にはトークン数で見えない。実際の使用感で示す。

| 1回の質問の規模                 | 消費         | 日50,000 | 月500,000 |
| ------------------------------- | ------------ | -------- | --------- |
| 典型（入力 2,000 + 出力 800）   | 2,800 tokens | 約17回   | 約178回   |
| 重い（入力 6,000 + 出力 2,000） | 8,000 tokens | 約6回    | 約62回    |

日次上限は「典型的な質問を1日17回」にあたる。学習の途中で詰まって連続して聞く日を
想定しても足りる一方、自動化されたループが回り続けた場合は数分で止まる。
...
**2027-01-01 に単価が倍になる。** 上限を tokens で固定しているため、
同じ上限のまま**上限額だけが2倍になる**。下の試算に 2027 年以降の列を併記してあるのは、
その日に何も気づかないまま請求が倍になるのを避けるためである。
**2026-12 中にこの節を見直す。**

### 上限額の試算

最悪ケースを2通りで取る。実際の請求はこの間に収まる。

| 想定                       | 1人あたり日次 | 1人あたり月次 | 2027-01-01 以降の月次 |
| -------------------------- | ------------- | ------------- | --------------------- |
| 入力:出力 = 7:3（現実的）  | $0.08         | **$0.83**     | $1.65                 |
| 全量が出力（理論上の最悪） | $0.19         | **$1.88**     | $3.75                 |

利用者数を掛けた運営側の月次上限は次のとおり。**上限が効いていれば**この額を超えない。

| 想定利用者数 | 月次の最大（最悪ケース） | 2027-01-01 以降 |
| ------------ | ------------------------ | --------------- |
| 50人         | $94                      | $188            |
| 200人        | **$375**                 | $750            |
| 1,000人      | $1,875                   | $3,750          |

**200人で月$375 を許容できる上限として採った。** 個人が運営する学習プロダクトとして、
不正利用が最大まで起きても月額が3桁ドルで止まることを条件にしている。
...
2. **学習イベントの同期と Learning Map の閲覧は通常どおり動かす。**
   上限はコストのある経路だけに掛かる。
   `learning-events:sync` と `learning-profile` に上限を適用しない。
3. **Copilot と BYOK へ案内して継続させる。** 停止ではなく経路の切り替えを促す。
...
- **モデルの allowlist 化。** 現在 `apps/api/src/routes/ai.ts` は `model` を
  任意の文字列で受け付ける。上限を tokens で固定しても、単価の高いモデルを
