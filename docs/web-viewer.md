...
**Auth/05（#84）で Identity を入れた後も、この判断は維持している。** ただし
API が 401 を返した場合は例外で、**KV のセッションを即座に削除する**。残したままだと
同じ Cookie を持つ別の誰かが refresh を回して使い続けられるため、伝播遅延を
許容できるのは「利用者自身のログアウト」までである（`src/worker/index.ts`）。
強整合が必要になったら Durable Objects をその時点で再評価する。
...
代わりに `auth_unavailable`（セッションは生きているが IdP が一時的に応答しない）が入った。
利用者への指示が「再ログイン」ではなく「再試行」になる場面を区別するためである。
...
```jsonc
{
  ...
  "assets": {
    ...
    "run_worker_first": ["/api/*", "/login", "/callback", "/logout", "/consent"],
  },
  "vars": {
