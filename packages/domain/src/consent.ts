 ...
 *   文面へ含めた（#157）
 * - 6: 「質問履歴の保存」オプトインで質問文・選択テキスト・回答本文が
 *   学習記録サーバーへ保存される経路を文面へ含めた（#204）
 */
export const CONSENT_NOTICE_VERSION = 6;
...
export const CONSENT_NOTICE_DETAIL = [
  ...
  "本プロダクトはこれらの本文を AI の送信経路では保存しません",
  "（「質問履歴の保存」のオプトインを除きます。送信先での扱いは、その提供元の規約によります）。",
  "",
  "【学習記録サーバーへ送るもの】",
  "Concept ID、言語、イベント種別、時刻、セッションID、端末ID。",
  "設定画面で入力した項目（表示名など）や Managed AI の利用回数・トークン量も含みます。",
  "「質問履歴の保存」を有効にしている場合のみ、質問文・選択テキスト・AI の回答が",
  "学習記録サーバーに保存されます。履歴はあなたが削除するまで残り、",
  "設定で無効にしても保存済みの履歴は残ります。",
  "無効の場合、コード本文・質問文・AIの回答を学習記録として保存することはありません。",
  "",
